...
dlmwrite('w_straight_back.txt',w)
dlmwrite('c_straight_back.txt',c)
dlmwrite('h_straight_back.txt',h)
...
%   Output : 
%-----------------------------------------------------------------------%
%goal = [-0.02 ,0.3];    %straight hit(test)
...
%y_t = [-0.024,0.1];     %straight hit(test)
%y_t = [-0.005,0.1285];  %straight hit(initial)
y_t = [0.001,0.2919];   %straight back(initial)
dy_t = zeros(n_dofs);
ddy_t = zeros(n_dofs);
z_t = 1.0;
f_t = [0, 0];
