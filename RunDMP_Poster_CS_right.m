...
% dlmwrite('w_right_hit.txt',w)
% dlmwrite('c_right_hit.txt',c)
% dlmwrite('h_right_hit.txt',h)
...
y_t = [0.0134,0.13];  %right hit(initial)
%y_t = [0,0.1];  %right hit(test)
%y_t = [0.121,0.261];  %right back(initial)
dy_t = zeros(n_dofs);
ddy_t = zeros(n_dofs);
z_t = 1.0;
f_t = [0, 0];
